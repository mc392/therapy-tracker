...
- **v9 (Sep 2026)** added `settings.reports` — the reports a therapist has built and saved, and which one is their default. A v9 backup can hold "CPCAB client log, everything so far, these nine sections, counted as one clinical hour each"; a v8 build has no such field, so it would drop every saved report and save that loss back. **Only the definitions are stored** — the figures are always rebuilt from the sessions, so an older build loses the saved report, never the data behind it.
...
- Schedule (ordered by threshold): **5** sessions *and at least one paid* → Receipts & statements · **10** → Tax + Costs & other income · **15** → Streaks & celebrations · **20** → Trends · **20** *and something logged to report on* → Reports · **40** → Table view.
...
Gated: `tax`, `finances`, `mtd`, `trends`, `accreditation`, `notesSync`, `reports` (`PLUS_FEATURES`). **`palettes` was dropped in Sep 2026** when colour schemes were switched off entirely — see Setup wizard § Palettes.
